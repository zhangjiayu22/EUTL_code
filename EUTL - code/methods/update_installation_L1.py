...
def update_installation_L1(collection, target_code, target_code_tr, target_code_ac, new_code):
    operator_map = {}
    for doc in collection.find(
            {"installation_id": {"$ne": None}},
            {"installation_id": 1, target_code: 1, "_id": 0}
    ):
        if doc.get(target_code):
            operator_map[doc["installation_id"]] = doc[target_code]
    ...
    for doc in transaction.find(
        ...
        tr_id = doc.get("tr_installation_id")
        ac_id = doc.get("ac_installation_id")
    ...
    account_cursor = Installation_L1.find({}, {"installation_id": 1, "_id": 0})
    bulk_updates = []
    count = 0

    for acc in account_cursor:
        installation_id = acc.get("installation_id")
        operator_val = operator_map.get(installation_id)
        tr_val = tr_map.get(installation_id)
        ac_val = ac_map.get(installation_id)
        ...
        if count % 2000 == 0:
            Installation_L1.bulk_write(bulk_updates, ordered=False)
            bulk_updates = []
            print(f"done writing {count}")

    if bulk_updates:
        Installation_L1.bulk_write(bulk_updates, ordered=False)


def update_installation_code_L1(collection, target_code, batch_size=1000):
    bulk_updates = []
    count = 0
    ...
    result = Installation_L1.update_many(
...
def update_installation_company_L1(batch_size=2000):
    tr_map = {}
    ac_map = {}
    ...
    bulk_updates = []
    count = 0
    total = Installation_L1.count_documents({})
    cursor = Installation_L1.find({}, {"installation_id": 1})
    ...
    # 5. 补全缺失字段，该字段不存在则补为空值
    Installation_L1.update_many(
...
def update_main_activity_L1(batch_size=2000):
    tr_map = {}
    ac_map = {}
