...
def update_installation(collection, target_code, target_code_tr, target_code_ac, new_code):
    operator_map = {}
    ...
    account_cursor = Installation_L2.find({}, {"installation_id": 1, "_id": 0})
    bulk_updates = []
    count = 0
    ...
    print(f"done writing {new_code},  {count} in total")
    result = Installation_L2.update_many(
        {new_code: {"$exists": False}},
        {"$set": {new_code: ""}}
    )
    print(f"update {new_code} as Null,  {result.modified_count} in total.")
